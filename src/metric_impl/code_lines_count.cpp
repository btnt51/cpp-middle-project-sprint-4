...
MetricResult::ValueType CodeLinesCountMetric::CalculateImpl(const function::Function &f) const {
    ...
    auto is_statement = [](std::string_view s) { return s.contains("_statement"); };

    auto count_parentheses = [](std::string_view s) { return std::ranges::count(s, '(') - std::ranges::count(s, ')'); };
    bool flag = false;

    auto blocks_view =
        ast | std::views::all | std::views::drop_while([&flag, &is_statement](const auto &s) {
            if (flag)
                return false;
            if (is_statement(s)) {
                flag = true;
        ...
        std::views::chunk_by([bal = 0, &count_parentheses](std::string_view /*prev*/, std::string_view cur) mutable {
            bal += count_parentheses(cur);
            return bal > 0;
        }) |
        std::views::filter([&is_statement](auto &&chunk) { return is_statement(*chunk.begin()); });
    ...
    int total = std::ranges::fold_left(blocks_view | std::views::transform(block_size), 1, std::plus<>{});
