...
MetricResult::ValueType CountParametersMetric::CalculateImpl(const function::Function &f) const {
    ...
    auto start_parametrs = std::ranges::find_if(ast, [](const auto& line) {
        return line.contains("parameters: (parameters");
    });

    auto end_parametrs = std::ranges::find_if(ast, [](const auto& line) {
        return line.contains("body: (block");
    });
    ...
    if (start_parametrs != ast.end() && end_parametrs != ast.end() && start_parametrs != end_parametrs) {
        ...
        auto param_count = std::ranges::count_if(range, [](const auto& line) {
