...
auto AnalyseFunctions(const std::vector<std::string>& files,
    ...
    auto all_functions = files | std::views::transform([](const auto& file_path) {
        return analyser::file::File(file_path);
    }) | std::views::transform([&](const auto &file) {
        return funcExtractor.Get(file);
    }) | std::views::join;
...
auto SplitByClasses(const auto& analysis) {
    auto grouped = analysis
    | std::views::chunk_by([](const std::pair<analyser::function::Function, analyser::metric::MetricResults> pair1, const std::pair<analyser::function::Function, analyser::metric::MetricResults> pair2) {
        return pair1.first.class_name < pair2.first.class_name;
    })
    | std::views::transform([](auto group) {
        return group | std::ranges::to<std::vector>();
    })
...
void AccumulateFunctionAnalysis(
    ...
    std::ranges::for_each(analysis, [&](const std::pair<analyser::function::Function, analyser::metric::MetricResults> & metric_results) {
